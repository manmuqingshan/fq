...
func (sm usacCoreSBRFramehRatioIndexSymMapper) MapScalar(s scalar.S) (scalar.S, error) {
	if e, ok := sm[s.ActualU()]; ok {
		s.Sym = e.ratioIndex
	}
	return s, nil
}
...
func (sm usacCoreSBRFramehOutputFrameLengthSymMapper) MapScalar(s scalar.S) (scalar.S, error) {
	if e, ok := sm[s.ActualU()]; ok {
...
var usacExtElementConfigMap = scalar.UToScalar{
...
func usacDecoderExtElementConfig(d *decode.D) {
	extType := d.FieldUFn("ext_type", decodeEscapeValueAddFn(4, 8, 16), usacExtElementConfigMap)
	d.FieldUFn("length", decodeEscapeValueAddFn(4, 8, 16))
	var defaultLength uint64
	if d.FieldBool("default_length_present") {
		defaultLength = d.FieldUFn("default_length", decodeEscapeValueAddFn(8, 16, 0))
	}
	// TODO: type
...
var usacElementTypeMap = scalar.UToScalar{
...
func usacDecoderConfig(d *decode.D, coreSbrFrameLengthIndex uint64) {
	numElements := d.FieldUFn("num_elements", decodeEscapeValueAddFn(4, 8, 16), scalar.UAdd(1))
...
var usacConfigExtensionMap = scalar.UToScalar{
...
func usacDecoderConfigExtension(d *decode.D) {
	numConfigExtensions := d.FieldUFn("num_config_extensions", decodeEscapeValueAddFn(2, 4, 8), scalar.UAdd(1))
	d.FieldArray("extensions", func(d *decode.D) {
		for i := uint64(0); i < numConfigExtensions; i++ {
			d.FieldStruct("extension", func(d *decode.D) {
				typ := d.FieldUFn("type", decodeEscapeValueAddFn(4, 8, 16), usacConfigExtensionMap)
				length := d.FieldUFn("length", decodeEscapeValueAddFn(4, 8, 16))
...
func usaccDecoder(d *decode.D, in interface{}) interface{} {
	d.FieldUFn("sampling_frequency", decodeEscapeValueAddFn(5, 24, 0), frequencyIndexHzMap)
	...
	if channelConfiguration == 0 {
		numOutChannels := d.FieldUFn("num_out_channels", decodeEscapeValueAddFn(5, 8, 16))
